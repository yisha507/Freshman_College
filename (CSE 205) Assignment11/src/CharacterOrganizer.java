...
public boolean putInSupportingStack(char character2)
 {
   ...
   for (int i = numberOfSupportingStacks-1; i >= 0 ; i--)
    {
      ...
      if(supportingStacks[i].isEmpty()==false && supportingStacks[i].peek().charValue() > character2) {
    	  if(bestTop > supportingStacks[i].peek().charValue()) {
    	  chosenStackIndex = i;
    	  bestTop = supportingStacks[i].peek().charValue();
    	  }
      } 
      else if(supportingStacks[i].isEmpty())
    	  chosenStackIndex = i;
    }
